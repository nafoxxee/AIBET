...
class TelegramSender:
    ...
    async def _show_system_status(self, message: Message):
        ...
        status_text = (
            ...
            f"• ML Models: {'✅ Trained' if self._check_ml_models() else '❌ Not trained'}\n\n"
            f"📊 Last Scan:\n"
            f"• CS2: {self._get_last_scan_time('cs2')}\n"
            f"• KHL: {self._get_last_scan_time('khl')}\n\n"
    ...
    # Helper methods for new UI functions
    def _check_ml_models(self) -> bool:
        """Check if ML models are trained"""
        try:
            import os
            cs2_model_path = "cs2/ml/models/classifier.pkl"
            khl_model_path = "khl/ml/models/classifier.pkl"
            return os.path.exists(cs2_model_path) and os.path.exists(khl_model_path)
        except:
            return False
    
    def _get_last_scan_time(self, sport: str) -> str:
        """Get last scan time for sport"""
        try:
            from storage.database import db_manager
            # This would be implemented to get actual scan times
    ...
    async def _show_cs2_scenarios(self, message: Message):
        ...
        try:
            ...
            from storage.database import db_manager
            # This would get recent scenarios from database
            ...
            await message.answer(f"❌ Error loading CS2 scenarios: {str(e)}")
    
    async def _toggle_cs2_alerts(self, message: Message):
        """Toggle CS2 alerts"""
        # This would implement alert toggling
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ Back", callback_data="cs2_menu")]
        ])
    ...
    async def _show_khl_scenarios(self, message: Message):
        ...
        try:
            ...
            await message.answer(f"❌ Error loading KHL scenarios: {str(e)}")
...
async def initialize_telegram():
    ...
    telegram_sender = TelegramSender(config.telegram.bot_token)
    return telegram_sender


async def main():
    """Main application entry point"""
    logger.info("Starting AI BET Analytics Platform")
    
    # Initialize Telegram
    telegram = await initialize_telegram()
    if not telegram:
    ...
    try:
        ...
        logger.warning(f"Sport modules not yet implemented: {e}")
